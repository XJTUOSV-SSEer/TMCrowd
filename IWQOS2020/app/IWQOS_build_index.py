...
def DeployContract(contract_path='../build/contracts/federated.json'):  ############# 修改合约名字
...
#读取kw-file关系
f_broker = open('/Users/yuzhemeng/PycharmProjects/IWQOS2020/broker.txt', 'rb')
broker=pickle.load(f_broker)
# print(broker)

#读取broker密钥
f_broker_key = open('/Users/yuzhemeng/PycharmProjects/IWQOS2020/broker_key.txt', 'rb')
...
###########################################添加任务索引
#读取添加的kw-file关系
# f_addbrokertask = open('/Users/yuzhemeng/PycharmProjects/IWQOS2020/addbrokertask.txt', 'rb')
...
def constructData(path):
    ...
    # cnt = 0
    for file in files:
        # participant_F.append('requester'+file)
        keyword_tid.append({})
        ...
        for tid_file in tid_files:
            ...
            with open(target, "r") as f:
                for line in f.readlines():
                    content = line.split(",")
                    for con in content:
                        if con in keyword_tid[-1]:
                            keyword_tid[-1][con].append(tid_file+'_'+file)
                        else:
                            keyword_tid[-1][con] = [tid_file+'_'+file]

constructData('/Users/yuzhemeng/PycharmProjects/IWQOS2020/dataset_test')



for i in range(len(broker_key)):
    # print('i',i)
    for kw in keyword_tid[i]:
        kkw = int.from_bytes(kw.encode(),byteorder='big',signed=False)
        a = broker_key[i][1]
        ac = ((mpz(kkw) % q) * (mpz(a) % q)) % q
        gac = gmpy2.powmod(g, ac, p)
        trap1 = hex(int(gac))
        for block in keyword_tid[i][kw]:
            # print(block)
            ...
            label1 = Web3.keccak(hexstr=trapdoor1)
            aes = AES.new(b'1234512345123451', model)
            # 生成token
            Tid = '0'*(32-len(block))+block
            C = aes.encrypt(Tid.encode())
            # 计算异或
            # G3 = label1.encode('utf-8')
            G3 = label1
            print(C.__len__(),G3.__len__())
...
for i in range(len(search_kw)):
    Tbw1, fbpie1= search(int.from_bytes(search_kw[i].encode(),byteorder='big',signed=False), broker_key[0][2], broker_key[0][0])
    token1 = int(Tbw1)
    ...
    ReturnC = store_var_contract.functions.get_returnC().call()
    print(ReturnC.__len__())
    set_tem.append(set())
    for C in ReturnC:
        # print(C.__len__())
        # tem_C = pad(int.from_bytes(C,byteorder='big',signed=False))
        # tid = aes.decrypt(tem_C.encode())
        set_tem[-1].add(C)
    clear_hash = store_var_contract.functions.clear_returnC().transact({
        "from": from_account,
        "gas": 80000000,
        "gasPrice": 0,
    })
    # clear_receipt = w3.eth.waitForTransactionReceipt(tx_hashkw1)
...
for ss in set_tem[0]:
    flag = True
    for i in range(len(set_tem)):
        if ss not in set_tem[i]:
            flag = False
            break
    if flag:
        cnt = cnt +1
